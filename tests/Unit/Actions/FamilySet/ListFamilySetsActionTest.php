...
describe('ListFamilySetsAction', function (): void {
    it('should return a collection', function (): void {
        // arrange
        $action = new ListFamilySetsAction;

        // act & assert - verify the action exists and is callable
        expect($action)->toBeInstanceOf(ListFamilySetsAction::class);
        expect(method_exists($action, 'execute'))->toBeTrue();

        // Note: The actual query behavior is tested through feature tests
        // which verify the correct family sets are returned for the authenticated user
    });
